...
void parser(string& s, size_t ignore_s, size_t ignore_e) {
    size_t globalID = 0;
    deque<size_t> stack;
    stack.push_front(0);
    CACHE = vector<size_t>(s.size() / 2 + 1, 0);
    size_t index = 0;
    for(auto c: s){
        if(c == '(') {
            globalID += 1;
            ...
            size_t tempIndex = stack.front();
            stack.pop_front();
            if(index < ignore_s || index > ignore_e) {
                CACHE[stack.front()] ^= (CACHE[tempIndex] + 1);
            }
        } else {
            exit(1);
...
int main() {
    ...
    if(risp) {
        for(size_t i = 1; i < ALL_BUCKETS.size(); i++) {
            parser(parentesi, ALL_BUCKETS[i].start, ALL_BUCKETS[i].end); //me no smart. me parse everything again
            size_t tryOne = CACHE[0];
