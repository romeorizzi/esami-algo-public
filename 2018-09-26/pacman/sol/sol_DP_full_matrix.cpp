 ...
 *  Soluzione di pacman con unico limite di allocare l'intera tabella di programmazione dinamica e dell'istanza invece che limitarsi a lavorare su due solo righe
 *
 *  Autore: Romeo Rizzi, 2018-09-24
 *
 */
...
// num[i][j][b] = number of paths starting from cell mappa[1][1] and arriving to cell mappa[i][j] with b units of residual blue (can still eat b ghosts after leaving cell mappa[i][j] without encountering any pills)
...
int main() {
	...
	if( (i==1) && (j==1) ) {
	  ...
	  switch (mappa[i][j]) {
	    ...
	    num[i][j][MAXBLUE] = 0;
	    for(int b = 0; b < MAXBLUE; b++)
	      num[i][j][b] = (num[i-1][j][b+1] + num[i][j-1][b+1]) % BASE;
	    num[i][j][0] = (num[i][j][0] + num[i-1][j][0]) % BASE;
	    num[i][j][0] = (num[i][j][0] + num[i][j-1][0]) % BASE;
	    break;
	    ...
	    for(int b = 0; b <= blue+1; b++) if(b <= MAXBLUE) {
	      num[i][j][blue] = (num[i][j][blue] + num[i-1][j][b]) % BASE;
	      num[i][j][blue] = (num[i][j][blue] + num[i][j-1][b]) % BASE;
	    }
	  }
	}
	//	for(int b = 0; b <= MAXBLUE; b++) 
