...
// implemento solo la simulazione (più facile, mi abbandono agli stili descrittivo e ricorsivo) e non mi preoccupo di fornire un conteggio più veloce del numero di mosse che andrebbero fatte
void sposta_disco(int n, char peg_from, char peg_to) {
  assert(curr_peg[n]==peg_from);
  if(t==1) printf("%smuove il disco %d dal piolo %c al piolo %c\n", player_name[player], n, peg_from, peg_to);
  curr_peg[n] = peg_to;
  numMoves += 1;
  player = 1-player;
}
