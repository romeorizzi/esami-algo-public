 ...
 * last change: 2018-06-15 author: Romeo Rizzi
 ...
 * 
 * if format=1 the tree is encoded as in the "specchio" problem but every node reports the number of its desendants rather than the number of its children. 
 * if format=2 the tree is encoded as in the "tree_transcode" problem. 
 * 
 */
...
int num_desc_on_close[MAX_N]; /*stores the tree encoded by listing in postorder the number of descendants, that is:
   num_descendants[i] = number of descendants of node i.
 (Where the IDs of the nodes are the first n natural numbers assigned in DFS-postorder starting from the first leaf as node 0 and the root as node n-1). */
...
// Scrive la depth d del nodo sia in apertura che in chiusura su depth[posW].
// Inoltre, solo in chiusura, scrive il numero di discendenti su num_desc_on_close[posW_on_close]
// Scrive inoltre la codifica in formato num_children di tale sottoalbero nel sotto-array num_children[root, root +n'-1], dove n' è il numero di nodi del sotto-albero radicato al nodo <root>
  depth[posW++] = d;
  num_children[root] = 0;
  ...
  depth[posW++] = d;
  num_desc_on_close[posW_on_close++] = num_descendants[root];
}
