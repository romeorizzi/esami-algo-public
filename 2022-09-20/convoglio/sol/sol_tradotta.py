...
for i in range(M):
    ...
    if i < N:
        out_nei[u].append(v+N) # the edges in the 1,1-correspondence are oriented from left to right
    else:
...
def dfs(v, dad, col):
    global color, out_nei
    """returns the triple (found,apex_node,finished) but also act on the two global variables above.
       It returns (found,apex_node,finished) = (False,None,False) when no alternating cycle has yet been found through the whole exploration of node v. Otherwise found=True, and assuming that also finished = True, then the alternating cycle has been entirely worked out and the global variable out_nei now contains the valid assignment obtained by applying the alternating cycle to the original assignment (i.e., the perfect matching contained in the input). Otherwise, found=True and finished=False and while an alternating cycle has been detected, the procedure is still backtracking and reversing itt arcs in order to update the assignment; in this situation, apex_node is the node where the cycle got closed and is needed internally to stop reconstructing the cycle and updating the assignment."""
