...
def lsample(Tot, K, Pop = xrange(0)):
    ...
    Add = [randlong((Tot.n if isinstance(Tot,lrange) else len(Tot))-len(Pop)-K+1) for i in xrange(K)]
    Add.sort()
    for i in xrange(K):
        Add[i] += i
...
class graph:
    ...
    # costruttore
    def __init__(self, N=0, E=None, type=None, w=None, nw=None, M=None, lbl=None, D=None):
        ...
        if D is not None and (not isinstance(D,int) or D < 1):
            raise StandardError("Incompatible parameters specified.")
        self.D = float("inf") if D is None else D # massima differenza tra estremi di un arco ammessa
        del D
        if isinstance(E,int):
            M=E
            E=None
        ...
        if len(E)==0 and (N > 1 or type == 'bipartite'):
            ...
            if type == 'connected':
                ...
                for i in xrange(1,N):
                    E.add(self.cod([randlong(max(i-self.D,0), i),i]))
                E |= sortedset(lsample(xrange(self.mMax()), M-N+1, E))
            if type == 'cycle':
                for i in xrange(N):
            ...
            if type == 'tree':
                for i in xrange(1,N):
                    E.add(self.cod([randlong(i),i]))
            if type == 'forest':
                if not (0 <= M < N):
                    raise StandardError("Parameter M out of bounds.")
                for i in lsample(N-1,M):
                    E.add(self.cod([randlong(i+1),i+1]))
            if type == 'clique':
                E = sortedset(xrange(self.mMax()))
            if type == 'star':
                for i in xrange(1,N):
                    E.add(self.cod([0,i]))
            if type == 'wheel':
            ...
            if type == 'bipartite':
                ...
                for i in xrange(NN[0]):
                    if idx[i] >= 0:
                        E.add(self.cod([randlong(NN[0],NN[0]+idx[i]+1),i]))
                for i in xrange(NN[1]):
                    if inv[i] >= 0:
                        E.add(self.cod([randlong(inv[i]+1),NN[0]+i]))
    ...
    def add(self, e):
        ...
        if isinstance(e[0], (list,tuple)):
            for ed in e:
                self.add(ed)
            return
        assert e[0] >= 0 and e[1] >= 0 and self.cod(e) >= 0
        if max(e[0],e[1]) >= self.V:
            self.lbl += range(self.V, max(e[0], e[1])+1)
...
class dag(graph):
    """Implementazione della classe graph per grafi diretti aciclici."""
    def cod(self, e):
        a=min(e[0],e[1])
        b=max(e[0],e[1])
        if b < self.D:
            return b*(b-1)/2 + a
        return (2*b-self.D)*(self.D-1)/2+a
    def dec(self, n):
        c = self.D*(self.D-1)/2
        if n < c:
            b = int(round(sqrt(2*(n+1))))
            a = n - b*(b-1)/2
            return [a,b]
        n -= c
        b = n / self.D
    ...
    def mMax(self):
        ...
        return self.D*(2*self.V-self.D-1)/2
        

class ldag(graph):
    """Implementazione della classe graph per grafi diretti aciclici con self-loops."""
    def cod(self, e):
        a=min(e[0],e[1])
        b=max(e[0],e[1])
        if b < self.D:
    ...
    def mMax(self):
        if self.V < self.D:
            return self.V*(self.V+1)/2
        return (2*self.V-self.D)*(self.D+1)/2
...
class ugraph(dag):
    ...
    def dec(self, n):
        a, b = dag.dec(self, n)
        return [a,b] if randlong(0,2) == 0 else [b,a]


class lugraph(ldag):
    """Implementazione della classe graph per grafi non diretti con self-loops."""
    def dec(self, n):
        a, b = ldag.dec(self, n)
        return [a,b] if randlong(0,2) == 0 else [b,a]
...
class dgraph(dag):
    ...
    def __init__(self, N=0, E=None, type=None, w=None, nw=None, M=None, lbl=None, D=None):
        """Comprende tutti i costruttori per graph, piu' type='dag' e type='fullclique'.
        ...
        if type == 'tournament':
            ...
            for i in xrange(N-1):
                for j in xrange(i+1,N):
                    E.add(self.cod([i,j] if randlong(0,2) == 0 else [j,i]))
            type = None
        graph.__init__(self,N,E,type,w,nw,M,lbl,D)
    def cod(self, e):
        return 2 * dag.cod(self, e) + (0 if e[0] < e[1] else 1)
    def dec(self, n):
        a, b = dag.dec(self, n/2)
        return [a,b] if n%2 == 0 else [b,a]
    def mMax(self):
        return 2 * dag.mMax(self)
    # grafo trasposto
    def __neg__(self):
...
class ldgraph(dgraph):
    """Implementazione della classe graph per grafi diretti con self-loops."""
    def cod(self, e):
        o = 0 if e[0] <= e[1] else 1
        a = min(e[0],e[1])
        b = max(e[0],e[1])
        if b < self.D:
            return b*b + 2*a + o
        return (2*b-self.D)*(self.D-1) + b + 2*a + o
    def dec(self, n):
        c = self.D*self.D
        if n < c:
            b = int(sqrt(n))
    ...
    def mMax(self):
        ...
        return (2*self.V-self.D)*(self.D+1) - self.V
